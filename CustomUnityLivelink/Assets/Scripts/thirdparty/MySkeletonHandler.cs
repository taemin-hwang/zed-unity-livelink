...
public class MySkeletonHandler : ScriptableObject
{
   ...
   // HumanBodyBones.LastBone, // Right Heel
    ...
    private MyZEDSkeletonAnimator zedSkeletonAnimator = null;
    ...
    public void Create(GameObject h, sl.BODY_FORMAT body_format)
    {
        ...
        zedSkeletonAnimator = humanoid.GetComponent<MyZEDSkeletonAnimator>();
    ...
    public void InitSkeleton(int person_id, Material skBaseMat)
    {
        ...
        int max_person_num = 8;
        string pid_str = (person_id % max_person_num + 1).ToString();

        // Debug.Log("pid_str: " + pid_str);
        number = GameObject.Find("number"+pid_str);
    }
    ...
    public void SetControlWithJointPosition(Vector3[] jointsPosition, Quaternion[] jointsRotation, Quaternion rootRotation, bool useAvatar, bool _mirrorOnYAxis)
    {
        ...
        if(MyZEDBodyTrackingManager.EnableSDKSkeleton) {
        ...
        if (useAvatar)
        {
            ...
            if (MyZEDBodyTrackingManager.EnableSDKSkeleton)
            {
                UpdateSkeleton(MyZEDBodyTrackingManager.OffsetSDKSkeleton);
            }
        }
        else
        {
    ...
    {
        if(number != null && MyZEDBodyTrackingManager.EnableSDKSkeleton) {
            number.transform.localScale = new Vector3(1f, 1f, 1f);
            number.transform.position = new Vector3(rigBone[HumanBodyBones.Hips].transform.position.x, rigBone[HumanBodyBones.Hips].transform.position.y + 1f, rigBone[HumanBodyBones.Hips].transform.position.z);
        }
...
#region upper body mode
    ...
    /// </summary>
    public void UpdateRotationsUpperBody()
    {
        MoveUpperBody();
    }
