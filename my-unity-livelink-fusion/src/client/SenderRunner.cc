...
SenderRunner::SenderRunner(std::shared_ptr<ConfigParser> config_parser) : config_parser_(config_parser), running(false) {
    init_params.depth_mode = sl::DEPTH_MODE::ULTRA;
    init_params.camera_fps = config_parser_->GetFPS();

    // Set the resolution
    // init_params.camera_resolution = sl::RESOLUTION::HD720;
    auto resolution = config_parser_->GetResolution();
    if (resolution == "HD2K")
